...
pub contract Domains: NonFungibleToken {
    ...
    init() {
        ...
        self.DomainsPrivatePath = PrivatePath(identifier: "flowNameServiceDomains") ?? panic("Could not set storage path")
        self.DomainsPublicPath = PublicPath(identifier: "flowNameServiceDomains") ?? panic("Could not set storage path")
        ...
        self.RegistrarPrivatePath = PrivatePath(identifier: "flowNameServiceRegistrar") ?? panic("Could not set storage path")
        self.RegistrarPublicPath = PublicPath(identifier: "flowNameServiceRegistrar") ?? panic("Could not set storage path")
        ...
        // self.account.save(<- self.createEmptyCollection(), to: Domains.DomainsStoragePath) // why Domains and not self?
        self.account.save(<- self.createEmptyCollection(), to: self.DomainsStoragePath) // why Domains and not self?
        ...
        self.account.link<&Domains.Registrar{Domains.RegistrarPublic}>
            (self.RegistrarPublicPath, target: self.RegistrarStoragePath)
    ...
    pub fun registerDomain(name: String,
            duration: UFix64, feeTokens: @FungibleToken.Vault, 
            receiver: Capability<&{NonFungibleToken.Receiver}>
        ) {
        let cap = self.account.getCapability<&Domains.Registrar{Domains.RegistrarPublic}>
            (Domains.RegistrarPublicPath)
        let registrar = cap.borrow() ?? panic("Could not borrow registrar public")
        return registrar.registerDomain(
            name: name,
            duration: duration, feeTokens: <- feeTokens, 
            receiver: receiver)
    }
    ...
    pub fun getRentCost(name: String, duration: UFix64): UFix64 {
        ...
        let cap = self.account.getCapability<&Domains.Registrar{Domains.RegistrarPublic}>
            (Domains.RegistrarPublicPath)
        let registrar = cap.borrow() ?? panic("Could not borrow registrar public")
        let price = registrar.getPrices()[len]
    ...
    pub resource NFT: DomainPublic, DomainPrivate, NonFungibleToken.INFT {
        ...
        pub fun setBio(bio: String) {
            ...
            pre {
                Domains.isExpired(nameHash: self.nameHash) : "Domain is expired"
            }
            self.bio = bio
    ...
    pub resource interface CollectionPrivate {
        access(account) fun mintDomain(name: String, nameHash: String, expiresAt: UFix64,
            receiver: Capability<&  {NonFungibleToken.Receiver}>)
    ...
    pub resource Collection: CollectionPublic, CollectionPrivate,
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            let oldToken <- self.ownedNFTs[id] <- domain // consequently do operations
    ...
    pub resource Registrar: RegistrarPublic, RegistrarPrivate {
        ...
        pub fun renewDomain(domain: &Domains.NFT, duration: UFix64, feeTokens: @FungibleToken.Vault) {
            ...
            if duration < self.minRentDuration {
                panic("Domain must be registered for at least the minimum duration: "
                    .concat(self.minRentDuration.toString()))
            }
            ...
            emit DomainRenewed(id: domain.id, name: domain.name, nameHash: domain.nameHash,
                expiresAt: newExpTime, receiver: domain.owner!.address)
        }

        pub fun registerDomain(name: String, duration: UFix64, feeTokens: @FungibleToken.Vault, 
            receiver: Capability<&{NonFungibleToken.Receiver}>) {
            ...
            self.domainsCollection.borrow()!.mintDomain(name: name, nameHash: nameHash, 
                expiresAt: expirationTime, receiver: receiver)
